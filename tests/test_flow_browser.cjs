...
(async()=>{
 ...
 try{
  ...
  await page.locator('#search').fill('金额');assert.notEqual(await page.locator('#search-count').textContent(),'0/0');assert(await page.locator('.node.match').count()>=1);await page.locator('#created').click();assert(await page.locator('#detail').isVisible());assert((await page.locator('#source').textContent()).includes('合成'));assert((await page.locator('#detail-text').textContent()).includes('100'));
  ...
  await page.locator('#search').fill('幂等');assert.notEqual(await page.locator('#search-count').textContent(),'0/0');await page.locator('#duplicate').click();assert((await page.locator('#detail-title').textContent()).includes('已处理'));await page.locator('.close').click();
