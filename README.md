...
下载的 HTML 可以离线阅读；网页内继续追问时，使用 Codex 提供的本机阅读地址。
...
![在伤害流程页面中追问固定伤害，直接阅读回答并选择整理或分享](docs/images/damage-qa.png)
...
- **只想继续了解细节**：留在网页里追问即可。
- **想把补充内容合到流程图里**：勾选需要的完整回答，点击 **整理新版本**。会生成新的 HTML，原版保留；这一步需要等待模型整理和流程结构核对。
- **想直接分享已有内容**：勾选回答，导出 **离线 HTML**，不需要重新调用模型。对方可以查看流程、节点详情和选中的问答，无需安装 Codex。
- **只需要一张图**：导出 **全图 PNG** 或 **当前视图 PNG**。图片保留画面，交互细节与问答请用离线 HTML 分享。
